...
class MusicGeneratorApp:
    ...
    def add_job_to_queue(self):
        """Raccoglie i parametri dalla GUI, incluse le opzioni di pulizia, e aggiunge un lavoro alla lista."""
        try:
            prompt, num_inst_len = self.build_prompt()
            if num_inst_len == 0: raise ValueError("Il prompt non contiene strumenti o una stima valida.")
            ...
            # --- RACCOLTA DELLE OPZIONI DI PULIZIA ---
            cleaning_options = {
                "remove_sustain": self.clean_remove_sustain_var.get(),
                "merge_tracks": self.clean_merge_tracks_var.get(),
                "trim_silence": self.clean_trim_silence_var.get(),
                "quantize": self.clean_quantize_var.get(),
                "quantize_grid": self.clean_quantize_grid_var.get(),
                "limit_polyphony": self.clean_limit_polyphony_var.get(),
                "polyphony_max": int(self.clean_polyphony_max_var.get()),
                "filter_pitch": self.clean_filter_pitch_var.get(),
                "pitch_min": int(self.clean_pitch_min_var.get()),
                "pitch_max": int(self.clean_pitch_max_var.get()),
                "filter_velocity": self.clean_filter_velocity_var.get(),
                "velocity_min": int(self.clean_velocity_min_var.get()),
            ...
            job_params = {
                ...
                "cleaning_options": cleaning_options, # <-- NUOVO DIZIONARIO AGGIUNTO
                "status": "In attesa"
            }
